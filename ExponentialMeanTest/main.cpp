...
int main()
{
    ...
    for(i = 0; i< NJobsArrived; i++)
    {
        ...
        double arrivalTime = lamdaDistribution(generator);
        clock = arrivalTime;
    ...
    while((NJobsArrived - 2*NWarmUpJobs) > warmUpJobsCounter)
    {
        warmUpJobsCounter++;
        Job CurrentJob = JobQueue.top();
        JobQueue.pop();
        ...
        case OLDJOB:
            ...
            while(!FoundServer && (numVisitedServers < NumberOfServers))
            {
                ...
                else if(Servers[currentServer].NumCurrentJobs < Servers[currentServer].Capacity)
                {
                    ...
                    Servers[currentServer].NumCurrentJobs++;
                    currentServer = (Servers[currentServer].ServerID + 1) % NumberOfServers;
                    numberOfHops[numVisitedServers+1]++;
                    Servers[currentServer].JobCount[(Servers[currentServer].NumCurrentJobs)]++;
                }
                numVisitedServers++;
            }

            if(numVisitedServers == NumberOfServers)
            {
               DropJobs++;
            }
            break;
